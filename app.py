...
<div class='banner'>
 ...
 st.error(f"Error: {st.session_state.status}")
 ...
 else "Select date in sidebar, then Fetch Weather, then Predict")
 st.info(msg)
 else:
...
""")
 ...
 st.markdown("<div class='sec'>Seasonal and Diurnal Patterns</div>",
 unsafe_allow_html=True)
 ca, cb = st.columns(2)
 with ca:
 fig = plot_monthly(df)
